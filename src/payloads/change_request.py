...
def create_block_message(text):
    """ Creates a block message with the given text. """
    return [
        {
            "type": "section",
            "text": {
...
def get_request_message(ticket_payload, context):
    """ Creates a block message for the given ticket payload and context. """
    message = [
        {
            "type": "section",
            "text": {
    ...
    add_field(message[2]["fields"], "Status", "Enable"
              if context["status"] == "true" else "Disable")
    add_field(message[2]["fields"], "Observations", context.get("observations", ""))
    return message

def create_request_review(context):
    """ Creates a request review view for the given context. """
...
def read_request_metadata(view):
    """ Reads the request metadata from the given view. """
